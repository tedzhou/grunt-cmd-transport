 ...
 *
 *   [{
 *			requireNode:[Uglifyjs.AST_Node],
 *			dependencies:['a', 'b'],
 *			factory:[Uglifyjs.AST_Function]
 *   },{
 *			requireNode:[Uglifyjs.AST_Node],
 *			dependenciesNode:['c', 'd'],
 *			factory:[Uglifyjs.AST_Function]
 *   }]
 */
function getAsyncRequires(ast) {
	ast = getAst(ast);

	var ret = [];
	var walker = new UglifyJS.TreeWalker(function (node) {
		if (node instanceof UglifyJS.AST_Call) {
			if (node.expression instanceof UglifyJS.AST_Dot && node.expression.property === 'async') {
				if (node.expression.expression instanceof UglifyJS.AST_SymbolRef && node.expression.expression.name === 'require') {
					var dependencies = [];
					if (node.args && (node.args[0] instanceof UglifyJS.AST_String)) {
						dependencies.push(node.args[0].value);
					} else if (node.args && node.args[0] instanceof UglifyJS.AST_Array) {
						node.args[0].elements.forEach(function(uglifyItem){
							if(uglifyItem instanceof UglifyJS.AST_String) {
								dependencies.push(uglifyItem.value);
							}
						});
					}
					var obj = {
						requireNode: node,
//						dependenciesNode: node.args[0],
						dependencies:dependencies
					};
					if (node.args[1] && node.args[1] instanceof UglifyJS.AST_Function) {
						obj.factory = node.args[1];
					}
					ret.push(obj);
				}
			}
...
exports.getAsyncRequires = getAsyncRequires;

function modifyAsyncRequire(ast, aimnode, options) {
		...
		if (node  === aimnode) {
			...
			if (node.expression instanceof UglifyJS.AST_Dot && node.expression.property === 'async') {
				if (node.expression.expression instanceof UglifyJS.AST_SymbolRef && node.expression.expression.name === 'require') {
					if (node.args && (node.args[0] instanceof UglifyJS.AST_String)) {
						ori = 1;
						node.args[0] = new UglifyJS.AST_Array({
							elements:dependencies.map(function(d){
								return new UglifyJS.AST_String({value:d});
							})
						});
					} else if (node.args && node.args[0] instanceof UglifyJS.AST_Array) {
						ori = node.args[0].elements.length;
						node.args[0] = new UglifyJS.AST_Array({
							elements:dependencies.map(function(d){
								return new UglifyJS.AST_String({value:d});
							})
					...
					if(node.args && (node.args[1])){
						node.args[1] = UglifyJS.parse('(function (){\n ('+node.args[1].print_to_string()+').apply(this, [].slice.call(arguments,0,'+ori+'))})').body[0].body;
					}
				}
			}
...
exports.modifyAsyncRequire = modifyAsyncRequire;
